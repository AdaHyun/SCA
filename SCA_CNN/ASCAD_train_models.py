...
def train_model(X_profiling, Y_profiling, model, save_file_name, epochs=150, batch_size=100,
    ...
    # EarlyStopping（仅在 early_stopping != 0 时启用）
    if early_stopping != 0:
    ...
    input_shape = None
    if hasattr(layer0, "batch_input_shape") and layer0.batch_input_shape is not None:
        input_shape = layer0.batch_input_shape
    elif hasattr(layer0, "input_shape") and layer0.input_shape is not None:
        input_shape = layer0.input_shape

    # fallback：新版 Keras 下 InputLayer 可能返回 None
    if input_shape is None:
        try:
            input_shape = model.input.shape
            print(f"[Info] input_shape fetched from model.input: {input_shape}")
        except Exception as e:
            print(f"[Warning] Cannot fetch model.input.shape: {e}")
            input_shape = (None, X_profiling.shape[1], 1)  # 兜底，假设 CNN 输入

    # ---------- 兼容多类型 ----------
    if isinstance(input_shape, (list, tuple)):
        input_layer_shape = input_shape
    else:
        input_layer_shape = (None, X_profiling.shape[1], 1)

    # ---------- Sanity Check ----------
    if input_layer_shape is not None and len(input_layer_shape) > 1:
        if input_layer_shape[1] != len(X_profiling[0]):
            print(f"[Warning] Model input shape {input_layer_shape[1]} != data length {len(X_profiling[0])}. Continue anyway.")
...
if __name__ == "__main__":
    if len(sys.argv)!=2:
        ...
        #MLP training
        # network_type = "mlp"
        training_model = "/root/autodl-tmp/SCA/Attack/CNN_torch/my_cnn_best_desync0_epochs75_batchsize200.h5"
